...
TOTAL_POINTS  = 200000
DATASET_TYPE  = "waymo"   # options: "nuscenes", "pandaset", "waymo", "kitti360"
MAX_FRAMES    = 98        # limit number of frames used
# =========================
...
if __name__ == "__main__":
    ...
    overlay_root = os.path.join(out_dir, "debug_ground_overlays_v6")
    ...
    for idx, frame in enumerate(tqdm(used_frames, desc="Building ground from cams 1/2/3")):
        ...
        # # --- dynamic mask (same pattern as non-ground script) ---
        ...
        #     mask &= dynamic_mask  # keep only non-dynamic pixels (or however your mask is defined)
    ...
    pcd.points = o3d.utility.Vector3dVector(points)
    pcd.colors = o3d.utility.Vector3dVector(colors)
    out_ply = os.path.join(out_dir, "ground_points3d_v6.ply")
    o3d.io.write_point_cloud(out_ply, pcd)
    print(f"Saved ground point cloud to: {out_ply}")
    ...
    print(high_level_commands)
    with open(os.path.join(out_dir, "ground_param_v5.pkl"), "wb") as f:
        pickle.dump((front_cam_poses, front_cam_height, high_level_commands), f)
