...
def main():
    ...
    for f in meta_data['frames']:
        ...
        smt = np.load(os.path.join(basedir, rgb_path.replace('images', 'semantics').replace('.jpg', '.npy')).replace('.png', '.npy'))
        # vehicle-like classes: car, truck, bus, train, motorcycle, bicycle
        car_mask = (smt == 11) | (smt == 12) | (smt == 13) | (smt == 14) | (smt == 15) | (smt == 18)
        mask = np.zeros_like(car_mask, dtype=bool)
        H, W = mask.shape

            # --- semantic-only fallback if no dynamics available ---
        dynamics = f.get('dynamics', {})
        if not dynamics:
                # No per-object boxes/poses: remove all vehicle-like pixels
            mask = car_mask.copy()
            print(f"Frame {rgb_path} has no dynamics info; using semantic mask only.")
        else:
            for iid, rt in dynamics.items():
                rt = np.array(rt)
            ...
            if valid_pixel.any():
                # (optional but safer) restrict to valid pixels before rounding
                xy = np.round(xy_screen[valid_pixel]).astype(int)
                ...
                cv2.fillPoly(bbox_mask, [xy_screen[[0, 1, 4, 5, 0]].astype(int)], 1)
                cv2.fillPoly(bbox_mask, [xy_screen[[2, 3, 6, 7, 2]].astype(int)], 1)
                cv2.fillPoly(bbox_mask, [xy_screen[[0, 2, 7, 5, 0]].astype(int)], 1)
                cv2.fillPoly(bbox_mask, [xy_screen[[1, 3, 6, 4, 1]].astype(int)], 1)
                cv2.fillPoly(bbox_mask, [xy_screen[[0, 2, 3, 1, 0]].astype(int)], 1)
                cv2.fillPoly(bbox_mask, [xy_screen[[5, 4, 6, 7, 5]].astype(int)], 1)
