...
# change from v13: accounting for distortion when verifying bounding boxes
# change from v14: saving undistorted images
# change from v15: same but dependent version
...
# OUTPATH    = "/workspace/Jack/HUGSIM/data/samplepoints_in_data/waymo_data/311238_part_0_new_v62"
...
def parse_camera_list(cam_arg: str):
    """
    Parse -c/--cameras argument.
    Accepts formats like:
      "1,2,3"
      "1 2 3"
      "[1,2,3]"
      "(1 2 3)"
      "FRONT,FRONT_LEFT"  (will be mapped via dataset_pb2.CameraName)
    """
    # Remove surrounding brackets/parentheses if present
    cam_arg = cam_arg.strip().strip("[]()")
    # Split on commas OR whitespace
    tokens = re.split(r"[,\s]+", cam_arg)
    ...
    for tok in tokens:
        ...
        if not tok:
            continue
        # numeric: interpret directly as enum value
        if tok.isdigit():
            cams.append(int(tok))
        else:
            # try name mapping via proto enum, e.g. "FRONT"
            name = tok.upper()
            if hasattr(dataset_pb2.CameraName, name):
                cams.append(getattr(dataset_pb2.CameraName, name))
            else:
                raise ValueError(f"Unrecognized camera token '{tok}' in -c/--cameras")
    return cams

if __name__ == '__main__':
    # ================== ARGPARSE CONFIG ==================
    parser = argparse.ArgumentParser(
        description="Load Waymo segment and export images, intrinsics, extrinsics, and dynamic meta_data.json"
    )
    parser.add_argument(
        "-b", "--base_dir",
        required=True,
        help="Base directory containing TFRecord segments (e.g. /path/to/split_311238_10parts)"
    )
    parser.add_argument(
        "-s", "--segment",
        required=True,
        help="Segment filename (e.g. 311238_part_0.tfrecord)"
    )
    parser.add_argument(
        "-o", "--out",
        required=True,
        help="Output directory for processed data/meta_data.json"
    )
    parser.add_argument(
        "-c", "--cameras",
        required=True,
        help='Camera IDs or names, e.g. "1,2,3" or "FRONT,FRONT_LEFT"'
    )
    parser.add_argument(
        "-d", "--downsample",
        type=float,
        default=2.0,
        help="Image downsample factor (default: 2.0)"
    )
    parser.add_argument(
        "-m", "--max_frames",
        type=int,
        default=None,
        help="Max frames to process (default: None = all frames)"
    )
    ...
    for rec in tqdm(it, desc="Waymo frames"):
        ...
        for img_pkg in frame.images:
            ...
            ############# Store undistored image ########
            # ---------- build K for this (downsampled) resolution ----------
            cal = ctx_cam_cals[cam]  # camera calibration (Waymo)
            fx = cal.intrinsic[0] / DOWNSAMPLE
            fy = cal.intrinsic[1] / DOWNSAMPLE
            cx = cal.intrinsic[2] / DOWNSAMPLE
            cy = cal.intrinsic[3] / DOWNSAMPLE
            ...
            # Distortion for this camera (k1, k2, p1, p2, k3)
            dist = dist_coeffs.get(cam, np.zeros(5, dtype=np.float32))

            # ---------- undistort image ----------
            # Get optimal new camera matrix for undistorted view
            newK, roi = cv2.getOptimalNewCameraMatrix(K3, dist, (w, h), alpha=0)

            img_undist = cv2.undistort(img, K3, dist, None, newK)
            ...
            cv2.imwrite(out_png, img_undist)
            ...
            # intr[cam].append(K)
                    # ---------- store intrinsics matching undistorted image ----------
            K4 = np.eye(4, dtype=float)
            K4[:3, :3] = newK.astype(float)
            intr[cam].append(K4)
    ...
    for dynamic_id, infos in dynamics.items():
        ...
        for idx, fid_global in enumerate(infos['frame']):   # fid_global = your current_global
            ...
            for cam in cams:
                ...
                K   = intr[cam][i_local]          # 4x4, we’ll use the 3x3 top-left
                ...
                # Use K (3x3) and per-camera distortion coeffs.
                K3 = K[:3, :3].astype(np.float32)
                dist = dist_coeffs.get(cam, np.zeros(5, dtype=np.float32))

                # Compute an "optimal" new camera matrix for this resolution
                # (this is primarily useful if you decide to undistort images;
                # here we still project onto the original distorted images).
                newK, roi = cv2.getOptimalNewCameraMatrix(K3, dist, (w, h), alpha=0)

                # points_cam are already in camera coordinates; use identity rvec/tvec
                pts_obj = points_cam.astype(np.float32).reshape(-1, 1, 3)
                rvec = np.zeros((3, 1), dtype=np.float32)
                tvec = np.zeros((3, 1), dtype=np.float32)

                # Project with distortion onto the (distorted) image plane
                pts_img, _ = cv2.projectPoints(pts_obj, rvec, tvec, K3, dist)
                pts_uv = np.round(pts_img.reshape(-1, 2)).astype(int)   # (8,2)
