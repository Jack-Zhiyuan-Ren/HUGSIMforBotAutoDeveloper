...
# BASE_PATH    = "/workspace/Jack/HUGSIM/raw_data"              # -b / --base_path
# SEGMENT      = "segment-1083056852838271990_4080_000_4100_000_with_camera_labels.tfrecord"      # -s / --segment
# CAMERAS      = [1,2,3]                         # -c / --cameras (e.g. [1, 2, 3, 4, 5])
# OUTPATH      = "/workspace/Jack/HUGSIM/data/samplepoints_in_data/waymo_data/1083056_2frame"                      # -o / --outpath
...
#                         help='If set, process at most this many frames')

#     # If you want to IGNORE command-line args and always use the values above:
#     args = parser.parse_args([])

#     # If instead you want CLI to still override these defaults, use:
#     # args = parser.parse_args()
...
#     # IMPORTANT: if WaymoDataFileReader does not reset its iterator,
#     # you may need to re-create it here. Keeping your original pattern:
#     datafile = WaymoDataFileReader(seq_path)  # <-- optional but usually safer

#     for frame_idx, frame in tqdm(enumerate(datafile)):
#         # NEW: stop once we've reached max_frames
...
#             if img_pkg.name not in ego_poses:
...
#### version with more recorded things
import os
import argparse
import open3d as o3d
import numpy as np
...
# ================== USER CONFIG (edit these) ==================
BASE_PATH    = "/workspace/Jack/HUGSIM/raw_data"              # -b / --base_path
SEGMENT      = "segment-10061305430875486848_1080_000_1100_000_with_camera_labels.tfrecord"      # -s / --segment
CAMERAS      = [1,2,3]                         # -c / --cameras (e.g. [1, 2, 3, 4, 5])
OUTPATH      = "/workspace/Jack/HUGSIM/data/samplepoints_in_data/waymo_data/1006130_v2"                      # -o / --outpath
DOWNSAMPLE   = 2.0                          # --downsample
# MAX_FRAMES   = 2                            # --max_frames
# ==============================================================
...
if __name__ == '__main__':
    ...
    frame_v2ws = []  # NEW: store frame.pose.transform per frame_idx
    
    # NEW: store raw 3D box parameters for each frame_idx
    boxes_per_frame = {}  # key: frame_idx, value: list of dicts
    ...
    for frame_idx, frame in tqdm(enumerate(datafile)):
        ...
        for img_pkg in frame.images:
            ...
            # v2w at each camera timestamp (img_pkg.pose.transform)
            v2w_img = np.array(img_pkg.pose.transform).reshape(4, 4)
            if img_pkg.name not in ego_poses:            # NEW: fix keying bug
                ego_poses[img_pkg.name] = []             # NEW
            ego_poses[img_pkg.name].append(v2w_img)      # NEW
        ...
        # ego pose from frame.pose.transform (shared for this frame across cams)
        v2w = np.array(frame.pose.transform).reshape(4,4)  # NEW (explicit)
        frame_v2ws.append(v2w)                             # NEW

        # 3d bbox
        # NEW: init container for this frame_idx
        if frame_idx not in boxes_per_frame:
            boxes_per_frame[frame_idx] = []
        ...
        for obj in frame.laser_labels:
            ...
            boxes_per_frame[frame_idx].append({
                "id": str(obj.id),
                "type": type_name,
                "hei    ght": float(height),
                "width": float(width),
                "length": float(length),
                "center_x": float(cx),
                "center_y": float(cy),
            ...
            # existing bottom-center logic for your dynamic pipeline
            x = cx
            y = cy
            z = cz - height / 2
    ...
    # normalize poses
    for cam, v2ws in ego_poses.items():
        for i, v2w_img in enumerate(v2ws):
            c2v = extr[cam][i]
            c2w = v2w_img @ c2v @ opengl2waymo
            if cam not in c2ws:
                c2ws[cam] = []
            c2ws[cam].append(c2w)
    inv_pose = np.linalg.inv(c2ws[1][0])
    ...
    for dynamic_id, infos in dynamics.items():
        ...
        obj_id = infos.get("obj_id", None)   # NEW
        ...
        for idx, fid in enumerate(infos['frame']):
            ...
            for cam in cams:
                ...
                valid_mask = (
                    (points_screen[:, 2] > 0) &
                    (points_uv[:, 0] >= 0) & (points_uv[:, 1] >= 0) &
                    (points_uv[:, 0] < w) & (points_uv[:, 1] < h)
                )
                if np.sum(valid_mask) > 0:
                    frame_visible = True
                    seq_visible = True
            ...
            if frame_visible:
                if fid not in rts:
                    rts[fid] = {}
                rts[fid][dynamic_id] = {"rt": rt.tolist(), "obj_id": obj_id}
    ...
    # assume camera 1 exists and defines number of frames
    for i in range(len(intr[1])):
        for cam in cams:
            intrinsic = intr[cam][i]
            camtoworld = c2ws[cam][i]
            ...
            # NEW: grab raw poses for this frame
            ego_pose_img = ego_poses[cam][i]          # img_pkg.pose.transform
            ego_pose_frame = frame_v2ws[i]            # frame.pose.transform
            ...
            info = {
                ...
                'timestamp': timestamps[i],
                "dynamics": rts.get(i, {}),
                "ego_pose_image": ego_pose_img.tolist(),   # NEW
                "ego_pose_frame": ego_pose_frame.tolist(), # NEW
                # NEW: all laser boxes for this frame index
                "laser_labels": boxes_per_frame.get(i, []),
            }
            meta_data['frames'].append(info)
